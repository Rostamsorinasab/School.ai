...
# Automatically initialize database if tables do not exist
try:
    import school_db
    if hasattr(school_db, 'ensure_db_initialized'):
        school_db.ensure_db_initialized()
    elif hasattr(school_db, 'init_db'):
        school_db.init_db()
        if hasattr(school_db, 'seed_data'):
            school_db.seed_data()
except Exception as e:
    import streamlit as st
